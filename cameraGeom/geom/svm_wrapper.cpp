...
#pragma message("******** SVM code hasn't been ported to OpenCV3 yet ***********")

#ifdef FINISH_UPDATING_OPENCV
static const int SVM_TYPE = cv::ml::SVM::NU_SVC;
...
public:
	...
	{
		if (savedState.getSignCorrection() != 0)
			svm.load(savedState.getSVMFilename());
	}
...
class CSVMTraining : public CSVMTraining_base
{
	...
	* */
	...
	double propCorrect(const cv::ml::SVM& svm,
		const cv::Mat& features,
		const cv::Mat& labels,
		double& dSignCorrection,
		double& dPrecision,
	...
	public:
		...
		double trainAndValidate(const cv::ml::SVMParams& svmParams, int& nNumSVs) const
		{
			cv::ml::SVM svm;
			const bool bVerbose = false;
			if (bVerbose) {
	...
	private:
		double validate(const cv::ml::SVM& svm) const
		{
			double dSignCorrection = 0, dPrecision = 0;
			return pTrainer->propCorrect(
				svm, aFeatures[eValidate], aLabels[eValidate], dSignCorrection, dPrecision, 0, 0, 0);
	...
	public:
		...
		{
			svmParams.svm_type = SVM_TYPE;
			svmParams.kernel_type = (gamma > 0) ? cv::ml::SVM::RBF : cv::ml::SVM::LINEAR;
			svmParams.class_weights = pClassWeights;
			svmParams.nu = nu;
			svmParams.C = nu;
			svmParams.gamma = gamma;
		...
		const cv::ml::SVMParams& getSvmParams() const
		{
			return svmParams;
		}
	...
	public:
		double trainOnAll(cv::ml::SVM& svm_final,
			const CSVMParameterisation& bestParameterisation,
			double& dSignFix,
			TPRLookup& adPrecision,
			CSigmoidParams& sigmoidParams,
	...
	std::vector<CSVMParameterisation> getHyperparamSets() const
	{
		...
		else if (SVM_TYPE == cv::ml::SVM::C_SVC) {
			for (double dPow = -5; dPow <= 15; dPow += 2)
				adNuVals.push_back(std::pow(2.0, dPow));
		}
